function [curveSampleECMHalf, curveSampleTitin, curveSampleTitinActive,...
  ...
  sampleQuadraticTitinCurves(sampleVectorHalfLength,...
...
for i=1:1:length(sampleVectorHalfLength)
  ...
  curveSampleECMHalf.y(i,1)=...
    calcQuadraticBezierYFcnXDerivative(xH, forceLengthECMHalfCurve,0);
  curveSampleECMHalf.dydx(i,1)=...
    calcQuadraticBezierYFcnXDerivative(xH, forceLengthECMHalfCurve,1);
  curveSampleECMHalf.d2ydx2(i,1)=...
    calcQuadraticBezierYFcnXDerivative(xH, forceLengthECMHalfCurve,2);
  curveSampleECMHalf.d3ydx3(i,1)=...
    calcQuadraticBezierYFcnXDerivative(xH, forceLengthECMHalfCurve,3);
  ...
  while(abs(lerr) > 1e-6 && iter < iterMax)
    lPH   = calcQuadraticBezierYFcnXDerivative(f,forceLengthProximalInverseCurve,0);
    dPH  = calcQuadraticBezierYFcnXDerivative(f,forceLengthProximalInverseCurve,1);

    lDH   = calcQuadraticBezierYFcnXDerivative(f,forceLengthDistalInverseCurve,0);
    dDH  = calcQuadraticBezierYFcnXDerivative(f,forceLengthDistalInverseCurve,1);
  ...
  curveSampleProximal.y(i,1)=...
    calcQuadraticBezierYFcnXDerivative(lPH,forceLengthProximalCurve,0);
  curveSampleProximal.dydx(i,1)=...
    calcQuadraticBezierYFcnXDerivative(lPH,forceLengthProximalCurve,1);
  curveSampleProximal.d2ydx2(i,1)=...
    calcQuadraticBezierYFcnXDerivative(lPH,forceLengthProximalCurve,2);
  curveSampleProximal.d3ydx3(i,1)=...
    calcQuadraticBezierYFcnXDerivative(lPH,forceLengthProximalCurve,3);   

  curveSampleDistal.x(i,1)=lDH;
  curveSampleDistal.y(i,1)=...
    calcQuadraticBezierYFcnXDerivative(lDH,forceLengthDistalCurve,0);
  curveSampleDistal.dydx(i,1)=...
    calcQuadraticBezierYFcnXDerivative(lDH,forceLengthDistalCurve,1);
  curveSampleDistal.d2ydx2(i,1)=...
    calcQuadraticBezierYFcnXDerivative(lDH,forceLengthDistalCurve,2);
  curveSampleDistal.d3ydx3(i,1)=...
    calcQuadraticBezierYFcnXDerivative(lDH,forceLengthDistalCurve,3);   
  ...
  iterMax=100;
  while(abs(lerr) > 1e-6 && iter < iterMax)
    ligpH   = calcQuadraticBezierYFcnXDerivative(f,forceLengthIgpInverseCurve,0);
    dligpH  = calcQuadraticBezierYFcnXDerivative(f,forceLengthIgpInverseCurve,1);

    lpevkH  = calcQuadraticBezierYFcnXDerivative(f,forceLengthPevkInverseCurve,0);
    dlpevkH = calcQuadraticBezierYFcnXDerivative(f,forceLengthPevkInverseCurve,1);            

    ligdH  = calcQuadraticBezierYFcnXDerivative(f,forceLengthIgdInverseCurve,0);
    dligdH = calcQuadraticBezierYFcnXDerivative(f,forceLengthIgdInverseCurve,1);            
  ...
  curveSampleIgp.y(i,1)=...
    calcQuadraticBezierYFcnXDerivative(ligpH,forceLengthIgpCurve,0);
  curveSampleIgp.dydx(i,1)=...
    calcQuadraticBezierYFcnXDerivative(ligpH,forceLengthIgpCurve,1);
  curveSampleIgp.d2ydx2(i,1)=...
    calcQuadraticBezierYFcnXDerivative(ligpH,forceLengthIgpCurve,2);
  curveSampleIgp.d3ydx3(i,1)=...
    calcQuadraticBezierYFcnXDerivative(ligpH,forceLengthIgpCurve,3);    

  curveSamplePevk.x(i,1)=lpevkH;
  curveSamplePevk.y(i,1)=...
    calcQuadraticBezierYFcnXDerivative(lpevkH,forceLengthPevkCurve,0);
  curveSamplePevk.dydx(i,1)=...
    calcQuadraticBezierYFcnXDerivative(lpevkH,forceLengthPevkCurve,1);
  curveSamplePevk.d2ydx2(i,1)=...
    calcQuadraticBezierYFcnXDerivative(lpevkH,forceLengthPevkCurve,2);
  curveSamplePevk.d3ydx3(i,1)=...
    calcQuadraticBezierYFcnXDerivative(lpevkH,forceLengthPevkCurve,3);     

  curveSampleIgd.x(i,1)=ligdH;
  curveSampleIgd.y(i,1)=...
    calcQuadraticBezierYFcnXDerivative(ligdH,forceLengthIgdCurve,0);
  curveSampleIgd.dydx(i,1)=...
    calcQuadraticBezierYFcnXDerivative(ligdH,forceLengthIgdCurve,1);
  curveSampleIgd.d2ydx2(i,1)=...
    calcQuadraticBezierYFcnXDerivative(ligdH,forceLengthIgdCurve,2);
  curveSampleIgd.d3ydx3(i,1)=...
    calcQuadraticBezierYFcnXDerivative(ligdH,forceLengthIgdCurve,3);  
