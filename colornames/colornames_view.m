...
% * alphanumerically (names include any leading indices).
% * alphabetically (names exclude any leading indices).
...
%
if nargin<1
	pnm = pnc{1+rem(round(now*1e7),numel(pnc))};
else
	assert(isChRo(palette),...
		'SC:colornames_view:palette:NotCharRowVector',...
...
trs = cellfun(@(s)perms(s(end:-1:1)),ucs,'UniformOutput',false);
trs = [ncs;cellstr(vertcat(trs{:}))];
acs = [ncs;ucs];
...
if nargin<2
	...
	assert(isChRo(order),...
		'SC:colornames_view:order:NotCharRowVector',...
		'The second input <order> must be a 1xN character vector')
	idz = strcmpi(order,trs);
	assert(any(idz),...
		'SC:colornames_view:order:UnknownOption',...
		'The second input <order> must be one of:%s\b.',sprintf(' %s,',trs{:}))
	srt = trs{idz};
end
%
...
set(pah,'Value',find(strcmpi(pnm,pnc)));
	...
	function cnvSortBy()
		...
		switch acs{idc}
			case 'AlphaNum'
				...
				return
			case 'Alphabet'
				[~,idx] = sort(lower(regexprep(cnc,'^[-+]?\d+\s*','')));
				return
			case 'RGB'
				mat = rgb;
			case 'HSV'
			...
			otherwise
				error('SC:colornames_view:space:UnknownOption',...,...
					'Colorspace "%s" is not supported. How did this happen?',srt)
		end
