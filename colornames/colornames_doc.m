...
%
% The color names are input as character row vectors and may supplied
% either within one cell array or as separate input arguments:
[cnc,rgb] = colornames('xkcd',{'red','green','blue'})
[cnc,rgb] = colornames('xkcd','eggshell','eggShell')
%% Match Index Number
% Palettes with a leading index number may be matched by just the number,
% or just the name, or both together:
colornames('CGA','9','LightBlue','9 Light Blue','9lightblue')
...
%% Match Character Diacritics
% Characters with diacritics will be matched with or without the diacritic:
colornames('SherwinWilliams','Rosé','Rose')
%% Match RGB
% Each input RGB triple is matched to the closest RGB triple from the
% requested palette:
[cnc,rgb] = colornames('HTML4', [0,0.2,1;1,0.2,0])
...
% # *|rgb|* : an Nx3 numeric matrix of RGB values, with values 0..scale.
% # *|names|* : an Nx1 cell array of character vectors, either written in
% CamelCase or as separate capitalized words, and sorted alphanumerically.
